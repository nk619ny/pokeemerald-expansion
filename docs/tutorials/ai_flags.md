...
## `AI_FLAG_ILLUSION_TRICKS`
Custom flag for trainers with an Illusion mon (e.g. Zoroark) in their party. Because Illusion disguises the sent-out mon as the last conscious mon in the party, the player can normally deduce which mon is "real" from the AI's predictable switch-in choices. With this flag, whenever the AI is deciding which mon to send out (after a KO or on a mid-turn switch) and it would choose either its benched Illusion mon or the mon Illusion would disguise it as (the mon in the back), there is a `SHOULD_ILLUSION_TRICK`% chance (default 33, configurable in `include/config/ai.h`) that it sends out the other one instead — so the player can't be sure whether the mon that appeared is the Illusion mon or the real thing. The swap only happens when it's legal (the other mon is alive, not on the field, not reserved by the partner's pending switch, and not protected by the Ace Pokémon flags) and only when Illusion would actually activate. Trainers without this flag are completely unaffected. All other switch logic (whether to switch, and the baseline choice of mon) is unchanged.

## `AI_FLAG_CUSTOM_STRATEGIES`
Dispatcher flag for hand-scripted, team-specific strategies the generic AI can't express. Each strategy is gated on a species/ability/item signature, so a trainer can carry this flag without affecting other trainers that use it for a different situation. Intended for double battles alongside `AI_FLAG_BASIC_TRAINER` or `AI_FLAG_SMART_TRAINER`.

The first strategy targets `TRAINER_GRUNT_MAGMA_HIDEOUT_14` (Bruxish + Coalossal). Bruxish deliberately pivots a Water move into its Coalossal partner to trigger Steam Engine (+Speed) and an un-triggered Weakness Policy (+Atk/SpA): **Flip Turn** when Bruxish is the absolute fastest mon on the field (so it pivots the frail Bruxish out for a stronger mon), otherwise **Aqua Jet** (priority guarantees the trigger before the player can KO either mon). The pivot is only forced if it won't KO Coalossal. Coalossal then scores its moves assuming the doubled offense (computed locally; the shared damage cache is untouched): in the Aqua Jet branch it prioritizes **Earthquake** to KO the now-expendable Bruxish and bring in a stronger partner (unless another move secures a KO Earthquake can't, or Earthquake isn't a 2HKO but another move is); in the Flip Turn branch Earthquake takes an extra penalty (it would likely KO the incoming replacement). Both combo mons suppress Terastal so Coalossal keeps its Water weakness. Once Weakness Policy is consumed and Steam Engine has fired, the signature no longer matches and scoring returns to normal. Tests: `test/battle/ai/ai_custom_strategies.c`.

The second strategy targets `TRAINER_EDGAR` (Abomasnow + Politoed). On turn 1 only, if Abomasnow has Soundproof and its Mega is still usable, and its partner knows Perish Song, a single roll (`RNG_CUSTOM_STRATEGIES`, shared with future Custom Strategies rolls) decides whether the team delays Abomasnow's Mega Evolution: `DELAY_MEGA_FOR_PERISH_SONG_CHANCE`% (default 50) normally, or `DELAY_MEGA_FOR_PERISH_SONG_ONE_SOUNDPROOF_CHANCE`% (default 10, no roll at all if both) if one/both of the player's Pokémon already have Soundproof. If adopted, Abomasnow's Mega is suppressed for the turn and it's strongly pushed toward **Protect**, while its partner is strongly pushed toward **Perish Song** — unless it has a fast kill available, which it takes instead. If not adopted (roll fails, or skipped because both player mons have Soundproof), Abomasnow Mega Evolves as normal and Perish Song is discouraged that turn. This only affects turn 1; from turn 2 onward scoring is unaffected either way. Tests: `test/battle/ai/ai_custom_strategies.c`.

The same `TRAINER_EDGAR` team also has three behaviors that apply beyond turn 1:
- **Post-Mega Abomasnow Protect gating**: once Abomasnow has actually Mega Evolved (gated on its Mega-form species, so this never overlaps with the turn-1 delay-mega logic above), Protect is only allowed when a player Pokémon threatens a fast kill on it *and* Protect isn't on cooldown (i.e. it wasn't used last turn) — and even then, only `MEGA_ABOMASNOW_PROTECT_CHANCE`% (default 40) of the time. In every other case Protect is heavily discouraged. A failsafe also blocks this if the Politoed partner has already locked in Soak targeting Abomasnow this turn (its type-based survival plan is already in motion, so Protect would just waste the turn).
- **Soak/Water Sport save an ally from a lethal Fire move**: Politoed (knows Soak) or Arctovish (knows Water Sport) heavily encourage the move (scored to land at fast-kill parity) when a known player Fire move would currently faint their ally, the ally would survive that same hit either as pure Water (Soak) or with Water Sport's Fire-damage reduction active, and the user outspeeds every player Pokémon that can currently faint the ally. A failsafe skips the encouragement if the ally has already locked in a protection move this turn.
- **Soak/Ice coordination failsafes**: Politoed won't Soak an enemy if its partner already locked in a non-Freeze-Dry Ice attack on that same enemy and Soaking it to pure Water would turn a secured KO into a resisted non-KO (recalculated with a temporary type override, not just a categorical block). Conversely, no Edgar teammate will fire a Water or non-Freeze-Dry Ice move into an enemy the partner already locked Soak onto if doing so would likewise cost a KO. Freeze Dry (`EFFECT_SUPER_EFFECTIVE_ON_ARG`) is exempt from both checks since it ignores the Water resistance. Tests: `test/battle/ai/ai_custom_strategies.c`.

The third strategy targets `TRAINER_YUJI` (Shuckle + Slowbro/Cofagrigus). The state is tracked by whether Shuckle still knows Mimic (it's permanently replaced once used, so its presence doubles as a "combo not yet landed" flag) rather than a fixed turn count, so the fallback below falls out naturally. On turn 1, the active Trick-Roomer (Slowbro or Cofagrigus, whichever partners Shuckle) is forced into **Trick Room** and Shuckle is forced into **Protect**. From turn 2 until Shuckle lands the copy, the Trick-Roomer is instead forced into **Body Press** against whichever foe it damages more, and Shuckle is forced into **Mimic** targeting that same ally — if Slowbro faints before the copy lands, Cofagrigus (once it qualifies as the active Trick-Roomer) is targeted instead. Once Shuckle has copied Body Press, it reverts to upkeep: **Protect** whenever Trick Room is down and Protect isn't on cooldown, and it will never target Cofagrigus with any move again. Each live Trick-Roomer likewise reverts to forcing Trick Room whenever it's down, deduped against the other Trick-Roomer ally (relevant only once Shuckle has fainted and both are out together) so they don't both use it the same turn. Tests: `test/battle/ai/ai_custom_strategies.c`.

The fourth strategy targets `TRAINER_CORA` (Shuckle). **Guard Split** is strongly forced onto the ally (both moves already legally target any battler, so this is purely a scoring decision) for as long as Shuckle hasn't yet had it resolve this battle — tracked via the existing move-history record (`gBattleHistory->usedMoves`), which persists across turns, rather than any new state. Once Guard Split has landed once, it reverts to normal scoring. Separately and unconditionally, **Acupressure** is never allowed to target Shuckle itself, though it remains free to target the ally under normal scoring. Tests: `test/battle/ai/ai_custom_strategies.c`.
