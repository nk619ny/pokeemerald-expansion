...
#define B_CUSTOM_INFATUATION_CHANCE 33 // custom infatuation nerf to balance removal of gender limit
...
#define B_CHAMPIONS_MOVE_UPDATES    TRUE       // Changes to moves reflecting the latest versions in Pokemon Champions
...
#define B_DEFOG_EFFECT_CLEARING     GEN_7 // In Gen5+, Defog does not lower Evasion of target behind Subsitute. In Gen6+, Defog clears Spikes, Toxic Spikes, Stealth Rock and Sticky Web from both sides. In Gen8+, Defog also clears active Terrain.
...
#define B_ILLUMINATE_ACC_BOOST      110        // Field-wide accuracy boost when Illuminate is on the field. 110 = 1.1x multiplier. Set to 100 to disable.
...
#define B_STEAL_WILD_ITEMS          GEN_8 // In Gen9+, Thief and Covet steal a wild Pokémon's item and send it to the bag. In Gens2-8, the stolen item would be held by the Thief/Covet user.
#define B_PREVENT_STEALING_WILD_ITEMS  TRUE //custom added
...
#define B_SAFARI_BALL_MODIFIER      GEN_8 // In Gen8+, Safari Ball's catch multiplier was reduced from x1.5 to x1.
...
#define B_ABILITY_WEATHER               GEN_5 // In Gen6+, ability-induced weather lasts 5 turns. Before, it lasted until the battle ended or until it was changed by a move or a different weather-affecting ability.
#define B_TERRAIN_PERMANENT             TRUE // If TRUE, terrain is permanent when set by abilities. 
