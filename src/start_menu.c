...
enum
{
    ...
    MENU_ACTION_BAG,
    MENU_ACTION_POKENAV,
    MENU_ACTION_FLY, //testing addition
    MENU_ACTION_PC, //testing addition
    MENU_ACTION_PLAYER,
    MENU_ACTION_SAVE,
    MENU_ACTION_OPTION,
    MENU_ACTION_EXIT,
...
static bool8 StartMenuFlyCallback(void); //testing addition
static bool8 StartMenuPCCallback(void); //testing addition
...
static const struct WindowTemplate sWindowTemplate_Time = {
    .bg = 0,
    .tilemapLeft = 1,
    .tilemapTop = 1,
    .width = 7,
...
static const struct MenuAction sStartMenuItems[] =
{
    ...
    [MENU_ACTION_FLY]             = {gText_MenuFly,     {.u8_void = StartMenuFlyCallback}}, //testing addition
    [MENU_ACTION_PC]              = {gText_MenuPC,     {.u8_void = StartMenuPCCallback}}, //testing addition
...
static void ShowTimeWindow(void)
{
    ...
    u8 timeStr[16];
    FormatDecimalTimeWithoutSeconds(timeStr, gLocalTime.hours, gLocalTime.minutes, FALSE);

    sTimeWindowId = AddWindow(&sWindowTemplate_Time);
    PutWindowTilemap(sTimeWindowId);
    DrawStdWindowFrame(sTimeWindowId, FALSE);
    AddTextPrinterParameterized(sTimeWindowId, FONT_NORMAL, timeStr, 4, 1, TEXT_SKIP_DRAW, NULL);
...
static bool8 StartMenuPCCallback(void) //added to support PC from Start Menu
{
    if (!gPaletteFade.active)
    {
        PlayRainStoppingSoundEffect();
...
static bool8 StartMenuFlyCallback(void) //added to support Fly from Start Menu
{
    if (!gPaletteFade.active)
    {
        PlayRainStoppingSoundEffect();
