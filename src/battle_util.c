...
bool32 TryChangeBattleWeather(enum BattlerId battler, u32 battleWeatherId, enum Ability ability)
{
    ...
    else
    {
        ...
        if (gBattleWeather & B_WEATHER_PRIMAL_ANY || ability != ABILITY_NONE)
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_DARK_CLOUDS: // Custom - creates Rain (Drizzle) and Electric Terrain (Electric Surge) on switch-in
                ...
                break;
            {
                ...
                bool32 terrainChanged = TryChangeBattleTerrain(battler, STATUS_FIELD_ELECTRIC_TERRAIN, TRUE);
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct BattleContext *ctx)
{
    ...
    if (IsMistyTerrainAffected(battlerDef, ctx->abilityDef, ctx->holdEffectDef, ctx->fieldStatuses) && moveType == TYPE_DRAGON)
        modifier = uq4_12_multiply(modifier, UQ_4_12(0.5));
    if (IsMistyTerrainAffected(battlerAtk, ctx->abilityAtk, ctx->holdEffectAtk, ctx->fieldStatuses) && moveType == TYPE_FAIRY)
    ...
    case ABILITY_ANALYTIC:
        if (GetBattlerTurnOrderNum(battlerDef) < GetBattlerTurnOrderNum(battlerAtk) && moveEffect != EFFECT_FUTURE_SIGHT)
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        break;
    ...
    case HOLD_EFFECT_SOUL_DEW:
        if ((gBattleMons[battlerAtk].species == SPECIES_LATIAS || gBattleMons[battlerAtk].species == SPECIES_LATIOS)
            && ((B_SOUL_DEW_BOOST >= GEN_7 && (moveType == TYPE_PSYCHIC || moveType == TYPE_DRAGON || moveType == TYPE_FAIRY))
...
static inline void MulByTypeEffectiveness(struct BattleContext *ctx, uq4_12_t *modifier, enum Type defType)
{
    ...
    if (GetMoveEffect(ctx->move) == EFFECT_SUPER_EFFECTIVE_ON_ARG && defType == GetMoveArgType(ctx->move) && !ctx->isAnticipation)
        mod = UQ_4_12(2.0);
    if (GetMoveEffect(ctx->move) == EFFECT_NEUTRAL_ON_ARG && defType == GetMoveArgType(ctx->move) && !ctx->isAnticipation)
        mod = UQ_4_12(1.0);
