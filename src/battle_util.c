...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    case EFFECT_TRIPLE_KICK:
        ...
        break;
    case EFFECT_SPIT_UP:
        // Power is fixed; use the value defined in move data
        break;
    case EFFECT_REVENGE:
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case HOLD_EFFECT_SOUL_DEW:
        ...
        {
            if ((gBattleMons[battlerAtk].species == SPECIES_LATIOS && (moveType == TYPE_PSYCHIC || moveType == TYPE_DRAGON))
             || (gBattleMons[battlerAtk].species == SPECIES_LATIAS && (moveType == TYPE_FAIRY || moveType == TYPE_DRAGON)))
                modifier = uq4_12_multiply(modifier, holdEffectModifier);
        }
        else if ((gBattleMons[battlerAtk].species == SPECIES_LATIAS || gBattleMons[battlerAtk].species == SPECIES_LATIOS)
              && !(gBattleTypeFlags & BATTLE_TYPE_FRONTIER) && IsBattleMoveSpecial(move))
        {
            modifier = uq4_12_multiply(modifier, holdEffectModifier);
        }
        break;
...
static uq4_12_t GetWeatherDamageModifier(struct DamageContext *ctx)
{
    ...
    if (attackerWeather & B_WEATHER_RAIN)
    {
        if (ctx->moveType != TYPE_FIRE && ctx->moveType != TYPE_WATER)
            return UQ_4_12(1.0);
        return (ctx->moveType == TYPE_FIRE) ? UQ_4_12(0.5) : UQ_4_12(1.3); //reduced sun boost of fire type attacks to 30%, similar to terrain
    }
