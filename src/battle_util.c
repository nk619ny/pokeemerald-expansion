...
bool32 TryChangeBattleTerrain(u32 battler, u32 statusFlag, bool32 isAbility)
{
    ...
    if (!(gFieldStatuses & statusFlag))
    {
        ...
        // Set permanent terrain (timer = 0) for abilities when B_TERRAIN_PERMANENT is TRUE
        if (B_TERRAIN_PERMANENT == TRUE && isAbility)
    ...
    case FIELD_EFFECT_OVERWORLD_WEATHER:
        if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED))
        {
            ...
            case WEATHER_SNOW:
                ...
                {
                    if (B_OVERWORLD_SNOW >= GEN_9 && !(FlagGet(FLAG_FORCE_SUN)))
                    {
            ...
            case WEATHER_FOG_HORIZONTAL:
                ...
                else if (B_OVERWORLD_FOG == GEN_4 && !(gBattleWeather & B_WEATHER_FOG))
                {
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_DARK_CLOUDS:
            if (TryChangeBattleWeather(battler, BATTLE_WEATHER_RAIN, TRUE) && TryChangeBattleTerrain(battler, STATUS_FIELD_ELECTRIC_TERRAIN, TRUE))
            {
        ...
        case ABILITY_PSYCHIC_SURGE:
            ...
            break;
        case ABILITY_DAMP:
            if (!(gFieldStatuses & STATUS_FIELD_WATERSPORT) && !gSpecialStatuses[battler].switchInAbilityDone)
            {
            ...
            break;
        case ABILITY_MUDDY:
            if (!(gFieldStatuses & STATUS_FIELD_MUDSPORT) && !gSpecialStatuses[battler].switchInAbilityDone)
            {
        ...
        case ABILITY_ZEN_MODE:
            if (!gSpecialStatuses[battler].switchInAbilityDone
             && (gBattleMons[battler].species == SPECIES_DARMANITAN_STANDARD || gBattleMons[battler].species == SPECIES_DARMANITAN_GALAR_STANDARD)
             && TryBattleFormChange(battler, FORM_CHANGE_BATTLE_SWITCH))
            {
                //gBattlerAttacker = battler; //seemingly not needed in latest version of expansion
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    case HOLD_EFFECT_LIGHT_BALL:
        if ((atkBaseSpeciesId == SPECIES_PIKACHU || atkBaseSpeciesId == SPECIES_PICHU) && (GetConfig(CONFIG_LIGHT_BALL_ATTACK_BOOST) >= GEN_4 || IsBattleMoveSpecial(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static uq4_12_t GetWeatherDamageModifier(struct DamageContext *ctx)
{
    ...
    if (ctx->weather & B_WEATHER_RAIN)
    {
        if (ctx->moveType != TYPE_FIRE && ctx->moveType != TYPE_WATER)
            return UQ_4_12(1.0);
        return (ctx->moveType == TYPE_FIRE) ? UQ_4_12(0.5) : UQ_4_12(1.3); //reduced rain boost of water type attacks to 30%, similar to terrain
    }
    if (ctx->weather & B_WEATHER_SUN)
    {
        if (ctx->moveType != TYPE_FIRE && ctx->moveType != TYPE_WATER)
            return UQ_4_12(1.0);
        return (ctx->moveType == TYPE_WATER) ? UQ_4_12(0.5) : UQ_4_12(1.3); //reduced sun boost of fire type attacks to 30%, similar to terrain
    }
    return UQ_4_12(1.0);
}
