...
// Collapses Morning/Evening into Day/Night, since the area screen only toggles between the two
static enum TimeOfDay NormalizeAreaTimeOfDay(enum TimeOfDay timeOfDay)
{
    return (timeOfDay == TIME_NIGHT || timeOfDay == TIME_EVENING) ? TIME_NIGHT : TIME_DAY;
}
...
void DisplayPokedexAreaScreen(enum Species species, u8 *screenSwitchState, enum TimeOfDay timeOfDay, enum PokedexAreaScreenState areaState)
{
    ...
    gAreaTimeOfDay = NormalizeAreaTimeOfDay(timeOfDay);
