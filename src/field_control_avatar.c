...
static const u8 *GetInteractedBackgroundEventScript(struct MapPosition *position, u8 metatileBehavior, enum Direction direction)
{
    ...
    case BG_EVENT_HIDDEN_ITEM:
        if (bgEvent->bgUnion.hiddenItem.underfoot == TRUE)
            return NULL;
        if (bgEvent->bgUnion.hiddenItem.hiddenItemId & HIDDEN_ITEM_DIRECT_FLAG)
            gSpecialVar_0x8004 = bgEvent->bgUnion.hiddenItem.hiddenItemId & ~HIDDEN_ITEM_DIRECT_FLAG;
        else
...
static const u8 *GetInteractedWaterScript(struct MapPosition *unused1, u8 metatileBehavior, enum Direction direction)
{
    ...
    {
        if (IsFieldMoveUnlocked(FIELD_MOVE_WATERFALL) && FlagGet(FLAG_BADGE08_GET) == TRUE && FlagGet(FLAG_RECEIVED_HM_WATERFALL) && IsPlayerSurfingNorth() == TRUE)
...
static bool32 TrySetupDiveDownScript(void)
{
    ...
    if (IsFieldMoveUnlocked(FIELD_MOVE_DIVE) && FlagGet(FLAG_BADGE07_GET) && FlagGet(FLAG_RECEIVED_HM_DIVE) && TrySetDiveWarp() == 2)
    {
...
static bool32 TrySetupDiveEmergeScript(void)
{
    ...
    if (IsFieldMoveUnlocked(FIELD_MOVE_DIVE) && FlagGet(FLAG_BADGE07_GET) && FlagGet(FLAG_RECEIVED_HM_DIVE) && gMapHeader.mapType == MAP_TYPE_UNDERWATER && TrySetDiveWarp() == 1)
    {
