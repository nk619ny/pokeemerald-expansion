...
static const u8 *GetInteractedWaterScript(struct MapPosition *unused1, u8 metatileBehavior, u8 direction)
{
    ...
    {
        if (FlagGet(FLAG_BADGE08_GET) == TRUE && FlagGet(FLAG_RECEIVED_HM_WATERFALL) && IsPlayerSurfingNorth() == TRUE)
...
static bool32 TrySetupDiveDownScript(void)
{
    ...
    if (FlagGet(FLAG_BADGE07_GET) && FlagGet(FLAG_RECEIVED_HM_DIVE) && TrySetDiveWarp() == 2)
    {
...
static bool32 TrySetupDiveEmergeScript(void)
{
    ...
    if (FlagGet(FLAG_BADGE07_GET) && FlagGet(FLAG_RECEIVED_HM_DIVE) && gMapHeader.mapType == MAP_TYPE_UNDERWATER && TrySetDiveWarp() == 1)
    {
