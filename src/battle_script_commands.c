...
static const u8 *const sMoveEffectBS_Ptrs[] =
{
    ...
    [MOVE_EFFECT_THUNDER_WAVE]     = BattleScript_MoveEffectParalysis,
};
...
static bool32 AccuracyCalcHelper(u32 move, u32 battler)
{
    ...
    if ((gStatuses3[battler] & STATUS3_ALWAYS_HITS && gDisableStructs[battler].battlerWithSureHit == gBattlerAttacker)
     ...
     || (B_TOXIC_NEVER_MISS >= GEN_6 && moveEffect == EFFECT_THUNDER_WAVE && IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_ELECTRIC))
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        if (statusChanged == TRUE)
        {
            ...
            if (gBattleScripting.moveEffect == MOVE_EFFECT_POISON
             ...
             || gBattleScripting.moveEffect == MOVE_EFFECT_THUNDER_WAVE
