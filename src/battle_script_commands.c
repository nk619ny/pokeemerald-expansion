...
{//   Item                      1+  11+  21+  31+  41+  51+  61+  71+  81+  91+   Levels
    ...
    { ITEM_NUGGET,      {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } }, // was Moon Stone
    { ITEM_NUGGET,       {   _,   3,   3,   4,   4,   4,   4,   5,   9,  10, } }, // was Sun Stone
    { ITEM_NUGGET,      {   _,   1,   1,   1,   1,   4,   4,   5,   4,   5, } }, // was Rare Candy
    { ITEM_NUGGET,          {   _,   _,   3,   4,   4,   4,   4,   5,   4,   5, } },
    { ITEM_MAX_POTION,      {   _,   _,   3,   4,   4,   4,   8,   8,   9,  30, } },
    { ITEM_MAX_ETHER,       {   _,   _,   1,   1,   4,   4,   4,   _,   _,   _, } },
    { ITEM_NUGGET,           {   _,   _,   1,   1,   1,   4,   4,   5,   4,   5, } }, // was PP UP
    { ITEM_BIG_NUGGET,      {   _,   _,   1,   1,   1,   1,   4,   5,   4,   5, } },
    { ITEM_DESTINY_KNOT,    {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } },
    { ITEM_NUGGET,       {   _,   _,   1,   1,   1,   1,   1,   1,   1,   1, } }, // was Leftovers
    ...
    { ITEM_NUGGET,      {   _,   _,   _,   _,   _,   _,   _,   1,   1,   1, } }, // was Bottle Cap
};
...
{
    if (GetWeather() & B_WEATHER_STRONG_WINDS || gFieldStatuses & STATUS_FIELD_CYCLONE_SHIFT)
    {
...
void SetMoveEffect(enum BattlerId battlerAtk, enum BattlerId effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    case MOVE_EFFECT_RECHARGE:
        ...
        // Power Herb skips the recharge turn
        if (GetBattlerHoldEffect(gEffectBattler) == HOLD_EFFECT_POWER_HERB)
        {
            BattleScriptPush(battleScript);
            gBattlescriptCurrInstr = BattleScript_PowerHerbSkipRecharge;
            break;
        }
    ...
    case MOVE_EFFECT_SYRUP_BOMB:
        if (!gBattleMons[effectBattler].volatiles.syrupBomb)
        {
            ...
            gBattleMons[effectBattler].volatiles.syrupBombSourceMove = MOVE_SYRUP_BOMB;
            BattleScriptPush(battleScript);
    ...
    case MOVE_EFFECT_CONSTRICT_SLOW:
        if (!gBattleMons[effectBattler].volatiles.syrupBomb)
        {
            ...
            gBattleMons[effectBattler].volatiles.syrupBombTimer = B_SYRUP_BOMB_TIMER;
            gBattleMons[effectBattler].volatiles.syrupBombIsShiny = IsMonShiny(mon);
            gBattleMons[effectBattler].volatiles.syrupBombSourceMove = MOVE_CONSTRICT;
            BattleScriptPush(battleScript);
...
static void Cmd_recoverbasedonsunlight(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].hp != gBattleMons[gBattlerAttacker].maxHP)
    {
        ...
        else if (GetBattlerAbility(gBattlerAttacker) == ABILITY_MEGA_SOL
              && (GetMoveEffect(gCurrentMove) == EFFECT_SYNTHESIS
               || GetMoveEffect(gCurrentMove) == EFFECT_MOONLIGHT
               || GetMoveEffect(gCurrentMove) == EFFECT_MORNING_SUN))
        {
            recoverAmount = 20 * GetNonDynamaxMaxHP(gBattlerAttacker) / 30;
        }
...
void BS_TrySetEscapePrevention(void)
{
    ...
    if (GetBattlerAbility(gBattlerTarget) == ABILITY_RUN_AWAY
