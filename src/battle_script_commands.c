...
static void Cmd_attackcanceler(void)
{
    ...
    if (GetMoveNonVolatileStatus(ctx.currentMove) == MOVE_EFFECT_PARALYSIS || GetMoveNonVolatileStatus(ctx.currentMove) == MOVE_EFFECT_THUNDER_WAVE)
    {
...
static bool32 HandleMoveEndMoveBlock(u32 moveEffect)
{
    ...
    case EFFECT_MISTY_EXPLOSION:
        if (!IsAbilityOnField(ABILITY_DAMP) && !IsAbilityOnField(ABILITY_MUDDY))
        {
