    ...
    if (gAiThinkingStruct->aiFlags[switchContext->battler] & AI_FLAG_SMART_SWITCHING)
    {
            ...
            && RandomPercentage(RNG_AI_SWITCH_YAWN, GetSwitchChance(SHOULD_SWITCH_YAWN)))
        {
            ...
            if (gBattleMons[switchContext->battler].statStages[STAT_EVASION] > (DEFAULT_STAT_STAGE + 3)
                ...
                && gAiLogicData->abilities[switchContext->opposingBattler] != ABILITY_ILLUMINATE
...
static s32 GetSwitchinWeatherImpact(enum BattlerId battler)
{
    ...
    if (holdEffect != HOLD_EFFECT_SAFETY_GOGGLES && ability != ABILITY_MAGIC_GUARD && ability != ABILITY_OVERCOAT)
    {
         ...
         && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY && ability != ABILITY_SUB_ZERO)
        {
            ...
            && ability != ABILITY_SAND_VEIL && ability != ABILITY_SAND_RUSH && ability != ABILITY_SAND_FORCE && ability != ABILITY_WIND_RIDER && ability != ABILITY_DESERT_SPIRIT) // Custom: Wind Rider and Desert Spirit immune to sandstorm damage
        {
            weatherImpact = maxHP / 16;
            if (weatherImpact == 0)
                weatherImpact = 1;
