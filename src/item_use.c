...
bool8 ItemfinderCheckForHiddenItems(const struct MapEvents *events, u8 taskId)
{
    ...
    for (i = 0; i < events->bgEventCount; i++)
    {
        ...
        if (events->bgEvents[i].kind == BG_EVENT_HIDDEN_ITEM
            && !FlagGet((events->bgEvents[i].bgUnion.hiddenItem.hiddenItemId & HIDDEN_ITEM_DIRECT_FLAG)
                ? (events->bgEvents[i].bgUnion.hiddenItem.hiddenItemId & ~HIDDEN_ITEM_DIRECT_FLAG)
...
static bool8 IsHiddenItemPresentAtCoords(const struct MapEvents *events, s16 x, s16 y)
{
    ...
    for (i = 0; i < bgEventCount; i++)
    {
        ...
        {
            if (!FlagGet((bgEvent[i].bgUnion.hiddenItem.hiddenItemId & HIDDEN_ITEM_DIRECT_FLAG)
                ? (bgEvent[i].bgUnion.hiddenItem.hiddenItemId & ~HIDDEN_ITEM_DIRECT_FLAG)
                : (bgEvent[i].bgUnion.hiddenItem.hiddenItemId + FLAG_HIDDEN_ITEMS_START)))
                return TRUE;
            else
                return FALSE;
        }
...
void ItemUseOutOfBattle_PokemonBoxLink(u8 taskId)
{
    if (gMapHeader.regionMapSectionId == MAPSEC_EVER_GRANDE_CITY
     ...
     || gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_ROUTE110_TRICK_HOUSE_ENTRANCE))
    {
        DisplayDadsAdviceCannotUseItemMessage(taskId, gTasks[taskId].tUsingRegisteredKeyItem);
        return;
    }
...
void ItemUseOutOfBattle_UnlimitedRepel(u8 taskId)
{
    ...
    if (!infiniteRepelOn)
    {
        ...
        if(gTasks[taskId].tUsingRegisteredKeyItem){
            ...
            DisplayItemMessage(taskId, 1, gText_UnlimitedRepelOn, CloseItemMessage);
        }
    }
    else
    {
...
void ItemUseOutOfBattle_RoamingBeacon(u8 taskId)
{
    ...
    static const struct { u8 group, num; } sRuggedMaps[] = {
        {MAP_GROUP(MAP_ROUTE111_NORTH), MAP_NUM(MAP_ROUTE111_NORTH)},
        {MAP_GROUP(MAP_ROUTE111_SOUTH), MAP_NUM(MAP_ROUTE111_SOUTH)},
        {MAP_GROUP(MAP_ROUTE111_DESERT), MAP_NUM(MAP_ROUTE111_DESERT)},
        {MAP_GROUP(MAP_ROUTE112), MAP_NUM(MAP_ROUTE112)},
        {MAP_GROUP(MAP_ROUTE113), MAP_NUM(MAP_ROUTE113)},
        {MAP_GROUP(MAP_ROUTE114), MAP_NUM(MAP_ROUTE114)},
        ...
        {MAP_GROUP(MAP_ROUTE135_NORTH), MAP_NUM(MAP_ROUTE135_NORTH)},
        {MAP_GROUP(MAP_ROUTE135_SOUTH), MAP_NUM(MAP_ROUTE135_SOUTH)},
        {MAP_GROUP(MAP_FORTREE_CITY), MAP_NUM(MAP_FORTREE_CITY)},
        {MAP_GROUP(MAP_FALLARBOR_TOWN), MAP_NUM(MAP_FALLARBOR_TOWN)},
        {MAP_GROUP(MAP_LILYCOVE_CITY), MAP_NUM(MAP_LILYCOVE_CITY)},
        {MAP_GROUP(MAP_LAVARIDGE_TOWN), MAP_NUM(MAP_LAVARIDGE_TOWN)},
        {MAP_GROUP(MAP_MAUVILLE_CITY), MAP_NUM(MAP_MAUVILLE_CITY)},
    };
    ...
    static const struct { u8 group, num; } sPlacidMaps[] = {
        ...
        {MAP_GROUP(MAP_ROUTE103), MAP_NUM(MAP_ROUTE103)},
        {MAP_GROUP(MAP_ROUTE103_WEST), MAP_NUM(MAP_ROUTE103_WEST)},
        {MAP_GROUP(MAP_ROUTE104_NORTH), MAP_NUM(MAP_ROUTE104_NORTH)},
        {MAP_GROUP(MAP_ROUTE104), MAP_NUM(MAP_ROUTE104)},
        {MAP_GROUP(MAP_ROUTE105), MAP_NUM(MAP_ROUTE105)},
        {MAP_GROUP(MAP_ROUTE106), MAP_NUM(MAP_ROUTE106)},
        {MAP_GROUP(MAP_ROUTE107), MAP_NUM(MAP_ROUTE107)},
        {MAP_GROUP(MAP_ROUTE115), MAP_NUM(MAP_ROUTE115)},
        {MAP_GROUP(MAP_ROUTE116), MAP_NUM(MAP_ROUTE116)},
        {MAP_GROUP(MAP_ROUTE117), MAP_NUM(MAP_ROUTE117)},
        {MAP_GROUP(MAP_ROUTE110), MAP_NUM(MAP_ROUTE110)},
        {MAP_GROUP(MAP_RUSTBORO_CITY), MAP_NUM(MAP_RUSTBORO_CITY)},
        {MAP_GROUP(MAP_VERDANTURF_TOWN), MAP_NUM(MAP_VERDANTURF_TOWN)},
        {MAP_GROUP(MAP_OLDALE_TOWN), MAP_NUM(MAP_OLDALE_TOWN)},
        {MAP_GROUP(MAP_PETALBURG_CITY), MAP_NUM(MAP_PETALBURG_CITY)},
...
{
    gItemUseCB = ItemUseCB_FullRestore;
    SetUpItemUseCallback(taskId);
}
