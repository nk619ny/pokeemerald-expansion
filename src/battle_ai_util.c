...
enum MoveTarget AI_GetBattlerMoveTargetType(enum BattlerId battler, enum Move move)
{
    ...
    if (effect == EFFECT_TERA_STARSTORM && gBattleMons[battler].species == SPECIES_TERAPAGOS_STELLAR)
        return TARGET_BOTH;
    if (IsDoubleBattle()
        && gAiLogicData->abilities[battler] == ABILITY_SPREAD_FISTS
        && IsPunchingMove(move)
    ...
    default:
    {
        ...
        for (u32 effectIndex = 0; effectIndex < additionalEffectCount; effectIndex++)
        {
            ...
            case MOVE_EFFECT_RECHARGE:
                    // Power Herb negates the recharge requirement
                    if (gAiLogicData->holdEffects[battlerAtk] == HOLD_EFFECT_POWER_HERB)
                        return FALSE;
...
static u32 GetWeatherDamage(enum BattlerId battlerId)
{
    ...
    if ((weather & B_WEATHER_SANDSTORM) && ability != ABILITY_WIND_RIDER && ability != ABILITY_DESERT_SPIRIT) // Custom: Wind Rider and Desert Spirit immune to sandstorm damage
    {
    ...
    if ((weather & B_WEATHER_HAIL) && ability != ABILITY_ICE_BODY && ability != ABILITY_SUB_ZERO)
    {
