...
static u32 Ai_SetMoveAccuracy(struct AiLogicData *aiData, u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (abilityAtk == ABILITY_NO_GUARD || abilityDef == ABILITY_NO_GUARD || abilityDef == ABILITY_ILLUMINATE || GetMoveAccuracy(move) == 0) // Moves with accuracy 0 or no guard ability always hit.
        accuracy = 100;
    else
    ...
    // other
        ...
        case EFFECT_FLATTER:
            if (!AI_CanConfuse(battlerAtk, battlerDef, aiData->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            break;
//        case EFFECT_PARALYZE: //effect_paralyze was removed
        case EFFECT_THUNDER_WAVE:
            ...
            if (!ShouldParalyze(battlerAtk, battlerDef, aiData->abilities[battlerDef]))
                ADJUST_SCORE(-5); //this change was added from MOVE_EFFECT_PARALYSIS
            break;
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SKILL_SWAP:
                if (aiData->abilities[battlerAtk] != aiData->abilities[BATTLE_PARTNER(battlerAtk)] && !attackerHasBadAbility)
                {
                    ...
                    if ((aiData->abilities[battlerAtk] == ABILITY_COMPOUND_EYES || aiData->abilities[battlerAtk] == ABILITY_KEEN_EYE || IsAbilityOnField(ABILITY_ILLUMINATE))
...
//        break; //removed in 1.12
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_HONEY:
            if (aiData->abilities[battlerDef] == ABILITY_HONEY_GATHER)
                ADJUST_SCORE(DECENT_EFFECT);
            break;
        ...
        default:
            if (GetMoveEffect(move) != EFFECT_BESTOW && aiData->items[battlerAtk] == ITEM_NONE && aiData->items[battlerDef] != ITEM_NONE)
            {
                ...
                case HOLD_EFFECT_BLACK_SLUDGE:
                    ...
                    break;
                case HOLD_EFFECT_HONEY:
                    if (aiData->abilities[battlerAtk] == ABILITY_HONEY_GATHER)
                        ADJUST_SCORE(DECENT_EFFECT);
                    break;
                case HOLD_EFFECT_IRON_BALL:
    ...
    case EFFECT_HEAL_BLOCK:
        if (AI_IsFaster(battlerAtk, battlerDef, move) && predictedMove != MOVE_NONE && IsHealingMove(predictedMove))
          ...
          || (aiData->holdEffects[battlerDef] == HOLD_EFFECT_HONEY && aiData->abilities[battlerDef] == ABILITY_HONEY_GATHER))
            ADJUST_SCORE(DECENT_EFFECT);
        break;
    case EFFECT_SOAK:
