...
#include "data/pokemon/elite_learnsets.h"
...
const u16 *GetSpeciesEliteLearnset(u16 species)
{
    ...
    if (learnset == NULL)
        return gSpeciesInfo[SPECIES_NONE].eliteLearnset;
    return learnset;
}
...
bool8 PokemonUseItemEffects(struct Pokemon *mon, u16 item, u8 partyIndex, u8 moveIndex, bool8 usedByAI)
{
    ...
    u32 maxAllowedEVs = !B_EV_ITEMS_CAP ? MAX_VITAMIN_EVS : GetCurrentEVCap();
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
             ...
             && GetMonData(mon, MON_DATA_LEVEL, NULL) != MAX_LEVEL)
            {
                ...
                else if (param == LEVEL_CAP) // New Cap Candy
                {
...
u8 GetRelearnerEliteMoves(struct Pokemon *mon, u16 *moves)
{
    u16 learnedMoves[MAX_MON_MOVES] = {0};
    u8 numMoves = 0;
    u16 species = GetMonData(mon, MON_DATA_SPECIES);
    ...
    if (eliteMoves[0] == MOVE_UNAVAILABLE)
        return numMoves;

    for (u8 i = 0; i < MAX_MON_MOVES; i++)
        learnedMoves[i] = GetMonData(mon, MON_DATA_MOVE1 + i, 0);

    for (u16 i = 0; eliteMoves[i] != MOVE_UNAVAILABLE; i++)
    {
        u16 j;
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
...
u8 GetNumberOfEliteMoves(struct Pokemon *mon)
{
    u16 moves[MAX_RELEARNER_MOVES] = {0};
    u16 species = GetMonData(mon, MON_DATA_SPECIES_OR_EGG, 0);
    ...
    return GetRelearnerEliteMoves(mon, moves);
}
